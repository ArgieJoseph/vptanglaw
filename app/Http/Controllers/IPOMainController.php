...
{     
      ...
      $acadrank_ft = DB::table('t_f_universities')
        ->join('universities','t_f_universities.u_id','=','universities.id')
            ->select('t_f_universities.id AS Id',
                     't_f_universities.u_id as UID',
                     'instructor As NoofInstructor',
                     'asst_prof AS AssistantProf',
                     'asso_prof AS AssociateProfessor',
                     'professor AS Professor')
                ->where('t_f_universities.id',$u)
                ->where('t_f_universities.sem_id',$sem)
                ->where('t_f_universities.sy_id',$sys)
                ->get();

                $acadrank_ft_data = json_decode( json_encode($acadrank_ft), true);

      return Excel::create($name,function($excel) use($acadrank_ft_data){
